...
async function buildSite() {
  ...
  const concurrency = config.server?.concurrency?.render || DEFAULT_CONCURRENCY;
  setStylesLimiter(pLimit(config.server?.concurrency?.styles || DEFAULT_CONCURRENCY));
...
async function buildSinglePage(pagePath) {
  const state = await ensureState();
  const limit = state.limiter || ((fn) => fn());
  return limit(() => state.renderer.renderPage(pagePath));
}
