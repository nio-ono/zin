...
async function* planStyles(ctx) {
  ...
  const scssFiles = allFiles
    .filter((file) => path.extname(file).toLowerCase() === '.scss' && !path.basename(file).startsWith('_'))
    .sort();
...
async function* planAssets(ctx) {
  ...
  const staticFiles = allFiles
    .filter((file) => shouldCopyStatic(file, sourceDir, ctx.config.server.directories.pages))
    .sort();
...
async function* planPages(ctx, pages) {
  ...
  const actions = await ctx.renderer.planPages(sortedPages);
  for (const action of actions) {
    yield action;
  }
}
...
async function* planSite(ctx, options = {}) {
  ...
  const pages = options.pages ? [...options.pages] : ctx.renderer.listPages();
