...
const { extractConfigBlock, parseJsonish } = require('./render/config-block');
const { codeFrame } = require('./errors');
...
function extractConfigAndContent(ejsContent, pagePath) {
  ...
  const allowJson = Boolean(config.server && config.server.allowConfigJSON);
  const configRegex = /<%[\s\n]*const\s+config\s*=\s*([\s\S]*?);[\s\n]*%>/;
  const matches = ejsContent.match(configRegex);
  let pageConfig = {};

  if (allowJson) {
    const block = extractConfigBlock(ejsContent);
    if (block) {
      try {
        pageConfig = parseJsonish(block);
      } catch (err) {
        console.error(`Error parsing @config block in ${pagePath}:`, err);
      }
...
function createRemoveAction(target, meta = { label: 'Removed' }) {
...
class Renderer {
  ...
  async planPage(pagePath) {
    ...
    try {
      ...
      html = pageRender.html;
    } catch (error) {
      await this.logRenderError(error, absPath);
      throw error;
    }
    ...
    if (entry.config && entry.config.template) {
      ...
      try {
        ...
        await this.logRenderError(error, templatePath, absPath);
        throw error;
      }
    }
    ...
    if (
      outputDir &&
      outputDir !== this.directories.publicDir &&
      !isPathInside(this.directories.publicDir, outputDir)
    ) {
    ...
    return [createWriteAction(outputFilePath, html, { label: 'Rendered' })];
  }

  async planPages(pagePaths) {
    const unique = [...new Set(pagePaths.map((page) => path.resolve(page)))];
    const actions = [];
    for (const target of unique) {
      const entryActions = await this.planPage(target);
  ...
  async planPageRemoval(pagePath) {
    const absPath = path.resolve(pagePath);
    ...
    if (entry.output && entry.output.outputDir) {
      return [createRemoveAction(entry.output.outputDir)];
    }
    return [];
  }

  async logRenderError(error, filePath, pagePath = filePath) {
    const targetFile = error.path || error.filename || filePath;
    const line = error.line || error.lineNumber || 1;
    const column = error.column || error.columnNumber || error.col || 0;
    console.error(`EJS error in ${targetFile}:${line}:${column}`);
    const frame = await codeFrame(targetFile, line, column);
    console.error(frame);
    if (pagePath && pagePath !== targetFile) {
      console.error(`Included from ${pagePath}`);
    }
  }
