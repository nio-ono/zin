...
async function processDirectory(currentDir, sourceDir, publicDir, paths, globals, collections) {
    ...
    for (const item of items) {
        ...
        if (stat.isDirectory()) {
            ...
            await processEJSFile(itemPath, sourceDir, publicDir, paths, globals, collections);
        }
    }
}

async function processEJSFile(itemPath, sourceDir, publicDir, paths, globals, collections) {
    ...
    let relativePath = path.relative(sourceDir, itemPath);
    ...
    const isInPagesDir = relativePath.startsWith('pages' + path.sep);

    if (isInPagesDir) {
        // Remove 'pages' from the path
