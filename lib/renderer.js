...
async function compileEJSToHTML(directories, globals) {
    try {
        ...
        // Pass baseSourceDir as sourceDir initially
        ...
        if (pagesDir !== sourceDir && await fs.pathExists(pagesDir)) {
            // Use the original sourceDir as baseSourceDir when processing pagesDir
...
async function processDirectory(currentDir, sourceDir, publicDir, directories, globals, collections, baseSourceDir) {
    ...
    for (const item of items) {
        ...
        if (stat.isDirectory()) {
            ...
            // Recursively process subdirectories with the same baseSourceDir
            ...
            const isTemplate = isPathInside(path.join(baseSourceDir, 'templates'), itemPath) || isPartial;
            ...
            // Process the EJS file with baseSourceDir
            await processEJSFile(itemPath, sourceDir, publicDir, directories, globals, collections, path.resolve(baseSourceDir, directories.pages), baseSourceDir);
        }
    }
}
...
async function processEJSFile(itemPath, sourceDir, publicDir, directories, globals, collections, pagesDir, baseSourceDir) {
    ...
    const includeFile = (filePath, options) => {
        const fullPath = path.resolve(baseSourceDir, filePath); // Use baseSourceDir instead of sourceDir
    ...
    try {
        const renderOptions = {
            filename: itemPath,
            root: baseSourceDir, // Use baseSourceDir here as well
            include: includeFile
        };

        if (pageConfig.template) {
            const templatePath = path.join(baseSourceDir, 'templates', `${pageConfig.template}.ejs`); // Use baseSourceDir
